<script lang="ts">
  ...
  interface Props {
    of: ComponentType<SvelteComponent<any, any, any>>;
    story: string;
    reloadable?: boolean;
  }

  let { of, story, reloadable = true }: Props = $props();
      ...
      <div class="form-group">
        <label class="control">
          ...
          {:else if control.type === "number"}
            <input id={key} {...control} bind:value={$meta.args[key]} oninput={onNumberInput(key, control)} />
          {/if}
        </label>
      </div>
    {/each}
