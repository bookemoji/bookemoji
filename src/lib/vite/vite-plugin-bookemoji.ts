// vite-plugin-bookemoji.js
// This file defines your custom Vite plugin.
import type { Plugin, ResolvedConfig } from "vite";

import * as path from "node:path";
import fg from "fast-glob"; // Fast and efficient globbing library
  ...
  return {
    ...
    async configResolved(resolvedConfig: ResolvedConfig) {
      ...
      try {
        ...
        console.log(plugin_prefix, "Attempting import of configFilePath:", configFilePath);

        await fs.access(configFilePath);

        // Dynamically import the user's config file.
        // We assume it's an ESM module exporting a default object.
        console.log(plugin_prefix, "Importing configFilePath:", configFilePath);
        ...
        if (svelteConfig && svelteConfig.default) {
          ...
          console.log(plugin_prefix, `Using custom glob pattern from svelte.config.js: "${userGlobPattern}"`);
        }
        ...
        if ((e as { code: string }).code === "ENOENT") {
          console.log(plugin_prefix, `svelte.config.js not found.`);
        }
      }
    },
    ...
    async load(id: string): Promise<string | null> {
      ...
      if (id === resolvedVirtualModuleId && isActive) {
        ...
        if (!projectRoot) {
          console.error(plugin_prefix, 'projectRoot not set. "configResolved" might not have run yet. Returning empty array.');
          return createExportStatements(bookEmojiConfig?.base ?? "", []);
        }

        console.log(plugin_prefix, `Searching for files with pattern "${userGlobPattern}" relative to "${projectRoot}"`);
        ...
        try {
          ...
          console.log(plugin_prefix, "full search path:", path.join(projectRoot, userGlobPattern));
          ...
          console.log(plugin_prefix, `${files.length} files found:`);
          files.forEach((f) => console.log(plugin_prefix, "\t- ", f));
